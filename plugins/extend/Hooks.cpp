...
static uint8_t Local_GetMeetsPrestigeClassRequirements() {
	...
	if(strncmp(GMPCR_pString->text, "SPECIALIST", 10) == 0) {
		int spec;
	...
	if(strncmp(GMPCR_pString->text, "DIVSPELL", 8) == 0) {
		int lvl;
	...
	if(strncmp(GMPCR_pString->text, "RACENOT", 7) == 0) {
		int race;
    ...
    if(strncmp(GMPCR_pString->text, "SCRIPT", 6) == 0) {
...
int Hook_DamageEffectListHandler (CNWSEffectListHandler *pThis, CGameObject *ob, CGameEffect *effect, int iArg) {
	...
	char * script = (char*)malloc(12 * sizeof(char));
	char * damager = (char*)malloc(11 * sizeof(char));
	nwn_objid_t creator = effect->eff_creator;
	
	CNWSScriptVarTable *vt;
	vt = &(((CNWSObject *)cre)->obj_vartable);
	script= (char*)"nwnx_damages";
	damager= (char*)"dmg_creator";
...
int Hook_OnDamage(CNWSEffectListHandler *handler, CNWSObject *obj, CGameEffect *effect, int arg){
	...
	CExoString *dmgr((char*)"dmg_creator");// = (CExoString *) malloc(sizeof(CExoString));
	...
	for (i=0; i< 12; i++) 
		{
			...
			CExoString *dmgVar((char*)"damage_"+i);// = (CExoString *) malloc(sizeof(CExoString));
