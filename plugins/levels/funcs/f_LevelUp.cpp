...
void Func_LevelUp(CGameObject *ob, char *value) {
    ...
    ls->ls_featlist.len   = feat_count;
    ls->ls_featlist.alloc = feat_count;
    ...
    for(j = 0; j < 10; j++){
        ...
        levels.Log(3, "Spells Gained Var Name: |%s|, Count: %d\n", var_name.text, spell_gain_count);
        for(i = 0; i < spell_gain_count; i++){
            ...
            levels.Log(3, "Spell : %d, Spell Level: %d\n", spell, j);
    ...
    for(j = 0; j < 10; j++){
        ...
        levels.Log(3, "Spells Removed Var Name: |%s|, Count: %d\n", var_name.text, spell_rem_count);
